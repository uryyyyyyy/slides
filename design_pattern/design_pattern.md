...
その結果コードが短くシンプルになるだけで、ムリヤリ行数を減らす（本来違うものをまとめる- 特殊な記述をする。）のは絶対NG


## デザインパターンの欠点。
現実世界を忠実にプログラミングする、
というオブジェクト指向の思想のもと、継承をガンガン使います。

また、覚えたての素人がデザインパターンを使おうとするとだいたい失敗するらしいです（理解が浅いため、使うべきでないところにも無理やり使うから。）

その結果、他の人が見て修正しづらい、追いにくくなるようです。
そういう意味では、デザインパターンは基本使わないでください。
後述しますが、これらを学ぶ意義は、その思想を理解することです。
...
メソッド関数を持ったクラスと、DTO VO entityなどは分けて考えよう。
よくある、アニマルクラスは吠えると食べるがあって〜とか、社員クラスは人クラスを継承して〜とかいらないから。普通は使わないから。

以降はこの単語を使っていこうと思います。 
データオブジェクト → DTO VO Entity フィールド変数と一部メソッド（GetterSetter、equals、hashcodeなどしか持たない。）
ロジックオブジェクト → フィールド変数を持たずメソッドだけ持つクラス Service Controller DAOがそれに該当する。
...
これを習得して下さい。
現場でこの思想と違うものがあったら適宜修正して下さい。

わかりやすくなかったら文句言いに来て下さい。
...
デメリットとして、継承先にかかれなくなるのでわかりにくい。
上にたどっていかなければいけない。
また、明確にすることとありますが、本当に明確ですか？
例えばIVｃDDを継承したSrwDateって言われてイメージつきますか？
「あ、あのメソッドとあのフィールド変数継承してるな」ってわかりますか？
全員が間違いなく理解できないなら使っちゃダメ、逆に混乱するから。
...
- ロジックの中身をそのまま使いたい→Bクラスに実装しないで、Aクラスに移譲すればいい
中身をそのまま実装したいというのは単一責務に反する。

--

以上、継承は使わない議論終わり。（これ話すには下地が必要だったので良かったです）
（継承をどうしても使いたい場合は、周りのエンジニア全員に
誰が見ても理解できるか了承を取ってから作って下さい。
